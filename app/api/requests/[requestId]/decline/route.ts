...
import { getRideRequestById, updateRideRequest, getTripById, getUserById } from '@/lib/store';
...
export async function POST(
  ...
  try {
    ...
    if (trip.driverId !== driverId) {
      return NextResponse.json(
        { error: 'Only the trip driver can decline requests' },
        { status: 403 }
      );
    }
    ...
    const updatedRequest = updateRideRequest(requestId, { status: 'declined' });
    if (!updatedRequest) {
      return NextResponse.json(
